...
void *dock(void *arg) {
    ...
    while (true) {
        ...
        randInterval = port.randInt(minServiceTime, maxServiceTime);
        printf("Dock %i is holding ship for %is\n", id, randInterval);
        usleep(randInterval * ONE_SECOND_US); // spend a service time
...
void *ships(void *arg) {
    ...
    int randInterval;
    int neededTugs = port.randInt(1, 3);

    while (true) { // Initiate customer's routine
        int dockId = -1;
        if ((dockId = port.dockShip(id, neededTugs)) != -1) {               // If ships was assigned to dock
            port.leaveDock(id, dockId);
        }
        randInterval = port.randInt(5, 15);
        printf("Ship %i has sailed for %is\n", id, randInterval);
        usleep(randInterval * ONE_SECOND_US); // Spend some time at hairdresse's
    }
